...
# 👇【追加】本番環境（URLキープ用）のパスワード認証ブロック [4, 5]
# ※ テスト用アプリにはこのブロックを入れず、本番用アプリにだけ入れます。
password = st.text_input("アクセスパスワードを入力してください", type="password")
if password != "your_secret_password":  # 👈 ここをお好きなパスワードに変更してください
    st.info("📢 現在、本番リリースに向けてシステム調整中です。")
    st.stop()  # パスワードが一致しない限り、これより下のコードは実行されません
# 👆ここまで
...
def load_dictionary():
    try:
        ...
        file_path = "master_data_final.json" 
...
POPULAR_TERMS = {
    "kaiseki": "【懐石 / 会席（かいせき） - Kaiseki】\nTraditional multi-course Japanese dinner, representing the pinnacle of Japanese culinary art.",
    "kappo": "【割烹（かっぽう） - Kappo】\nA high-end dining style where the chef cooks in front of the guests across a counter.",
    "tetsubin": "【鉄瓶（てつびん） - Tetsubin】\nTraditional Japanese cast iron kettle, known for mellowing water and lasting a lifetime.",
    "hocho": "【包丁（ほうちょう） - Hocho】\nJapanese culinary knives, world-renowned for their sharpness and traditional forging techniques.",
    "binchotan": "【備長炭（びんちょうたん） - Binchotan】\nPremium Japanese white charcoal, favored by top chefs for its clean, high-heat burning.",
    "donabe": "【土鍋（どなべ） - Donabe】\nTraditional Japanese clay pot used for cooking rice and stews, retaining heat exceptionally well.",
    "wagyu": "【和牛（わぎゅう） - Wagyu】\nPremium Japanese beef celebrated for its extensive marbling, tenderness, and rich flavor.",
    "omakase": "【お任せ（おまかせ） - Omakase】\nA dining style where guests leave the entire menu selection to the chef's expertise."
}
...
lang_options = {
    ...
    "Монгол (Mongolian)": "mn-MN", "নেपाली (Nepali)": "ne-NP", "Norsk (Norwegian)": "no-NO", "فارسی (Persian)": "fa-IR",
...
# 👇 Popular Termsのリンクボタン配置調整（上4・下4、末尾縦棒）
st.markdown(f"""
<div style="background-color: {tips_bg_color}; padding: 10px; border-radius: 6px; margin-bottom: 15px; font-size: 15px; line-height: 2.0; text-align: left;">
<b>🔥 Popular Terms:</b><br>
<a href="?word=kaiseki" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">kaiseki</a> |
<a href="?word=kappo" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">kappo</a> |
<a href="?word=tetsubin" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">tetsubin</a> |
<a href="?word=hocho" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">hocho</a> | <br>
<a href="?word=binchotan" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">binchotan</a> |
<a href="?word=donabe" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">donabe</a> |
<a href="?word=wagyu" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">wagyu</a> |
<a href="?word=omakase" target="_self" style="text-decoration: none; color: #3498db; font-weight: bold; margin: 0 5px;">omakase</a> |
</div>
""", unsafe_allow_html=True)
...
with st.form(key='chat_form'):
    ...
    # 👇【作戦1】自作マイクボタンを削除し、スマホ標準マイクの案内へ変更（英語）
    st.markdown(f"""
    <div style="margin-bottom: 10px;">
        <div style="display: flex; align-items: center; background-color: {tips_bg_color}; padding: 12px 15px; border-radius: 6px; border-left: 4px solid #2ecc71;">
            <div style="font-size: 14px; color: {html_text_color}; font-family: sans-serif; line-height: 1.5;">
                🎤 <b>For Voice Input:</b><br>
                Tap the search box and press the "microphone icon" on your smartphone keyboard to speak.
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)

# 👇【作戦3】強制キーボード隠し魔法（JS）を完全撤去（フリーズの原因を排除）
if submit_button:
    if user_input:
        with st.spinner(ui["thinking"]):
...
if st.session_state.answer:
    ...
    if st.session_state.get("found_item") and st.session_state.found_item.get("has_column"):
        ...
        romaji_word = romaji_words if romaji_words else "washoku" # URL用の修正
        target_url = f"https://nihongosound.com/en/gallery/{romaji_word}"
        st.link_button("🖼️ View Photos & Column", target_url)
        
    # 👇【作戦2】JavaScriptプレイヤーを削除し、Streamlit標準の安全プレイヤー（st.audio）へ変更
    st.write("---")
    st.markdown(f"**{ui['voice']}**")
    
    # 翻訳テキストから音声を生成 (無料の gTTS を使用)
    tts_lang = lang_code.split('-')
    if lang_code == "zh-CN": tts_lang = "zh-CN"
    elif lang_code == "zh-TW": tts_lang = "zh-TW"
    elif lang_code == "pt-BR": tts_lang = "pt-BR"
    elif lang_code == "pt-PT": tts_lang = "pt-PT"

    try:
        clean_text = answer.replace('*', '').replace('#', '').replace('\n', ' ')
        tts = gTTS(text=clean_text, lang=tts_lang)
        
        # 音声データをメモリ上に保存して表示（iPhoneでも確実に出る！）
        audio_fp = io.BytesIO()
        ...
        st.warning("⚠️ Voice generation is currently unavailable. Please try again later.")
